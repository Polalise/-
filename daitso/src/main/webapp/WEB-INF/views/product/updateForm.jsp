...
<form action="update.po" method="post" name="frm" enctype="multipart/form-data">
	<input type="hidden" name="p_num" value="${product.p_num }">
	<input type="hidden" name="pageNum" value="${pageNum }">
<table class="table table-bordered table-hover">
	<tr><td>제목</td><td><input type="text" name="p_name" required="required"
		autofocus="autofocus" value="${product.p_name }"></td></tr>
<!-- 	회원게시판에서 사용안하고 id나 이름을 가지고 fk설정
	회원게시판은 login하지 않으면 조회하지만 입력/수정/삭제는 불가능 비회원 게시판은 암호를 확하여 맞으면 수정/삭제 가능	 -->
	<tr><td>작성자</td><td><input type="text" name="p_writer" required="required"
		value="${product.p_writer }"></td></tr>
	<tr><td>가격</td><td><input type="text" name="price" required="required"></td></tr>
	<tr><td>지역</td><td><input type="text" name="p_local" required="required"
		value="${product.p_local }"></td></tr>
	<tr><td>판매/구매</td><td>
...
<!-- 	비회원 게시내용 종료 -->
	<tr><td>내용</td><td><textarea rows="5" cols="40" name="p_content" 	
		required="required">${product.p_content }</textarea></td></tr>
	<tr><td colspan="2" align="center">
	<input type="submit" value="확인" class="btn btn-info"></td></tr>
</table>
</form>
</div>
</body>
