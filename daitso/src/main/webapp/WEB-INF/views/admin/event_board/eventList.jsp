			...
			<c:if test="${not empty list }">
				<c:forEach var="event" items="${list }">
					...
					<tr>
						...
						<c:if test="${event.e_del != 'y' }">
							<td title="${event.e_content}"><a
								href="eventView.do?e_num=${event.e_num }&pageNum=${pb.currentPage}&id=${event.id}"
								class="btn btn-info btn-sm">${event.e_subject }</a> <!-- 조회수 늘리면 hot.gif 보여줘 -->
...
<select name="search">
   <c:forTokens var="sh" items="id,e_subject,e_content,subcon" delims="," varStatus="i">
